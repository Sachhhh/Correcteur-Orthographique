...
int main(int argc, char* argv[]){
  ...
  long double moyenne2=0;
    ...
    for (int i=0; i<repeat; i++){
      hashtable dico=lecture_dico((char*)argv[2]);
      long double cl = clock();
  ...
  printf("Temps moyen d'éxecution pour créer le dico sur %d répétitions avec la table de hashage : %.3Lf s\n",repeat,moyenne1);
  printf("Temps moyen d'éxecution pour verifier le texte sur %d répétitions avec la table hashage : %.3Lf s\n",repeat,moyenne2);
  return EXIT_SUCCESS;
}
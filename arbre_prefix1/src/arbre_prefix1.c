...
//Fonction qui alloue et retourne un arbre contenant la lettre passé en argument
...
//Fonction qui permet d'insérer le mot  dans l'arbre *racine
void inserer_arbre(noeud** racine,char * mot){
    ...
    for (int i = 0; i < longeur; i++) {
        ...
        if(lettre_est_presente(p,tolower(mot[i]))==0){
            inserer_tete(creer_arbre(tolower(mot[i])),&p->fils); 
        }
        //On desecend dans l'arbre avant d'ajouter la lettre suivante
        p=lettre_est_presente(p,tolower(mot[i]));   
    }
    if(p->fin_mot){
...
/*  On continue de descendre dans l'arbre même si on a affiché un mot puisque le mot affiché peut 
    ...
    while(p->fils!=NULL){
            ...
            newprefix[longueur]=p->fils->arbre->lettre;
            //On appel la fonction affichage pour l'arbre fils
            affichage(p->fils->arbre,newprefix,longueur+1);
...
bool rechercher_mot(noeud* arbre, char* mot){
    ...
    for(i=0; i<longueur ;i++){
        n=lettre_est_presente(p,tolower(mot[i]));
        if(n==0){
            return false;
        }
        p=n;
...
//Renvoie l'arbre fils contenant la lettre passé en argument si la lettre est présente parmis les fils,sinon renvoie 0 
noeud* lettre_est_presente(noeud* arbre,char lettre){
    ...
    liste l=arbre->fils;
    while(l!=NULL){
        if(tolower(lettre)==l->arbre->lettre){
            return l->arbre;
        }
        l=l->suivante;
    }
